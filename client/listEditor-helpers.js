...
Template.listEditor.helpers({
    'records': function(){
        Session.set('currentList', this._id);
        Meteor.subscribe('recordsList', Session.get("currentList"));
    ...
    'isEmpty': function(){
        var currentList = Session.get("currentList");
        return RecordsList.find({listId: currentList}).count() == 1;
    },
    'endOfBlock' : function(){
    ...
    'totalSumOfBlock' : function(){
        ...
        if(previousRecord != undefined) {
            while(previousRecord != undefined){
                if(previousRecord.isParent == true && RecordsList.findOne({_id: previousRecord.nextId}) != undefined && RecordsList.findOne({_id: previousRecord.nextId}).parentId == previousRecord._id){
                    var sumOfBlock = 0;
                    var hoursOfBlock = 0;
                    var childRecord = RecordsList.findOne({_id: previousRecord.nextId});
                    while(childRecord.nesting > previousRecord.nesting){
                        sumOfBlock = sumOfBlock + parseInt(childRecord.sum);
            ...
            var newBlock = {sum: this.sum, hours: this.hours, name: this.text, nesting: this.nesting};
            totalSum.push(newBlock);
        }
