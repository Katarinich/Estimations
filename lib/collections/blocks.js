...
Meteor.methods({
	blockUpdate: function(blockId, updates) {
		...
		if(updates.rate != undefined || updates.hasOwnProperty("hours") || updates.percent != undefined || updates.checked != undefined || updates.isParent != undefined) {
			...
			Blocks.find({estimationId: currentBlock.estimationId, nonDevelopment: undefined}).forEach(function(element) {
			...
			Blocks.find({estimationId: currentBlock.estimationId, nonDevelopment: true}).forEach(function(element) {
				if(element.isParent != true){
		...
		Meteor.call('estimationUpdate', currentBlock.estimationId, {dateUpdated: new Date()});
	},
	blockInsert: function(estimationId, parentId, nesting, index) {
		...
		Meteor.call('estimationUpdate', estimationId, {dateUpdated: new Date()});
	},
	blockRemove: function(blockId) {
		...
		Meteor.call('estimationUpdate', blockToRemove.estimationId, {dateUpdated: new Date()});
	},
	blockDepose: function(blockId, direction) {
    	...
    	var previousBlock = Blocks.findOne( {parentId: parentBlock._id, index: blockToDepose.index - 1} );
    	if(newNesting <= 4 && newNesting > 0 && !(blockToDepose.index == 0 && newNesting - parentBlock.nesting.substr(blockToDepose.nesting.length - 1) > 1)){
			...
			Meteor.call("blockUpdate", newParent._id, {isParent: true});
			Meteor.call("blockUpdate", blockToDepose._id, {index: newIndex, parentId: newParent._id, nesting: newNesting});
    	}
	}
});